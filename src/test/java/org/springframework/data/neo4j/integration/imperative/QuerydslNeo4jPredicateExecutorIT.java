...
class QuerydslNeo4jPredicateExecutorIT {
	...
	void fluentFindOneShouldWork(@Autowired QueryDSLPersonRepository repository) {
		...
		Person person = repository.findBy(predicate, org.springframework.data.repository.query.FluentQuery.FetchableFluentQuery::oneValue);
	...
	void fluentFindAllShouldWork(@Autowired QueryDSLPersonRepository repository) {
		...
		List<Person> people = repository.findBy(predicate, org.springframework.data.repository.query.FluentQuery.FetchableFluentQuery::all);
	...
	void fluentExistsShouldWork(@Autowired QueryDSLPersonRepository repository) {
		...
		Predicate predicate = Expressions.predicate(Ops.EQ, firstNamePath, Expressions.asString("Helge"));
		boolean exists = repository.findBy(predicate, org.springframework.data.repository.query.FluentQuery.FetchableFluentQuery::exists);
	...
	void fluentCountShouldWork(@Autowired QueryDSLPersonRepository repository) {
		...
		Predicate predicate = Expressions.predicate(Ops.EQ, firstNamePath, Expressions.asString("Helge"))
				.or(Expressions.predicate(Ops.EQ, lastNamePath, Expressions.asString("B.")));
		long count = repository.findBy(predicate, org.springframework.data.repository.query.FluentQuery.FetchableFluentQuery::count);
